...
#         st.write(f"{stock} downloaded data:", data.head())
...
#     st.write("Available columns in stocks_df:", stocks_df.columns)

    if 'Date' in stocks_df.columns:
         stocks_df['Date'] = stocks_df['Date'].astype('datetime64[ns]')
    else:
         st.error("⚠️ 'Date' column not found in stocks_df. Likely due to failed download or empty data.")
    ...
    #stocks_list = [stock for stock in stocks_list if stock in beta]
