...
def filter_data(dataframe, num_period):
    if num_period == '1mo':
       date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(months=-1)
        ...
        data = datetime.datetime(dataframe.index[-1].year, 1, 1).strftime('%Y-%m-%d')
    else:
...
def close_chart(dataframe, num_period = False):
    ...
    fig.update_layout(height = 500, margin = dict(l=0, r=20, t=20, b=0), plot_bgcolor='white', paper_bgcolor='#eeeeff', legend=dict(yanchor = "top", xanchor = "right"))

    return fig

def candle_chart(dataframe, num_period):
    dataframe = filter_data(dataframe, num_period)
    fig = go.figure()
    ...
    fig.update_layout(showlegend = False, height = 500, margin = dict(l=0, r=20, t=20, b=0), plot_bgcolor='white', paper_bgcolor='#eeeeff')
    return fig 
...
def RSI(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(
        x=dataframe['Date'],
        y=[30] * len(dataframe), name = 'Oversold', marker_color='green',line = dict(width=2, dash='dash',color='#79da84')
    ))
    fig.update_layout(yaxis_range=[0,100],
        height = 200, plot_bgcolor = 'white', paper_bgcolor = '#eeeeff', margin = dict(l=0, r=0, t=0, b=0), legend = dict(orientation='h', yanchor='top', y=1.02, xanchor='right', x=1))
    return fig
...
def MACD(dataframe, num_period):
    ...
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['MACD_Signal'], name='Overbought', marker_color='red', line=dict(width=2, color='red', dash='dash')))
...
def Moving_average_forecast(forecast):
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=forecast.index[:-30], y=forecast['Close'].iloc[:-30], mode='lines', name='Close Price', line=dict(width=2, color='black')
    ))
    fig.add_trace(go.Scatter(
        x=forecast.index[-31:], y=forecast['Close'].iloc[-31:], mode='lines', name='Future Close Price', line=dict(width=2, color='red')
    ))
    fig.update_xaxes(rangeslider_visible=True)
    fig.update_layout(height=500, margin=dict(l=0, r=20, t=20, b=0), plot_bgcolor='white', paper_bgcolor='#eeeeff', legend=dict(yanchor="top", xanchor="right"))
...
def candlestick(dataframe, num_period=None):
    if num_period:
        dataframe = filter_data(dataframe, num_period)
